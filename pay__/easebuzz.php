       ...
       // $ENV = "test";    // setup test enviroment (testpay.easebuzz.in)."KSBKZVZXK7";"IG3QOGGSCO";
        ...
        if($apiname === "initiate_payment"){
          ...
          //  print_r($_POST);
            ...
            */
                /*
            if(isset($_SESSION['send_check_pack'])){
                $packages= implode(', ', $_SESSION['send_check_pack']);
                echo "$packages";
            }else{
            ...
            $con=mysqli_connect("localhost", "root", "", "one") or  die("connection failed".mysqli_errno($con));
            $query="SELECT `trans_id` FROM `two` WHERE `username`='$user' AND `packages`='$packages'";
            ...
            */
            ...
            echo '<h1>You called wrong API, Pleae try again or contact us.</h1>';
        }

    }else{
        header('Refresh: 2; URL=http://deb.local/E-com2/packages.php');
        echo '<h1>Your Sessions Seems To Be Empty.You will Be Redirected </h1>';
    }
